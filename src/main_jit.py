...
from denoiser import Denoiser, DenoiserCustom
...
def get_args_parser():
    ...
    parser.add_argument('--iter_schedule', default='linear', choices=['constant', 'linear', 'cosine', 'concave', 'convex']) # N axis
    parser.add_argument('--stop_t', type=float, default=0.5)
    parser.add_argument('--seed_delta', type=int, default=42)
    ...
    parser.add_argument('--data_path', default='./data/imagenet', type=str,
                        help='Path to the dataset') # not using for eval (=fid measure)
...
def main(args):
    ...
    if args.evaluate_gen:
        ...
        with torch.random.fork_rng():
            ...
            with torch.no_grad():
                ...
                eval_path = evaluate(model_without_ddp, args, 0, batch_size=args.gen_bsz, log_writer=log_writer)
                eval_end_time = time.time()
        eval_elapsed_time = eval_end_time - eval_start_time
        pretty_time = str(datetime.timedelta(seconds=eval_elapsed_time))
        print(f"Total elapsed time: {pretty_time}")    

        time_txt_path = os.path.join(eval_path, 'elapsed_time.txt')
    ...
    # Training loop - We do not use here after 
